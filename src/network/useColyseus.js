...
export function useColyseus(countdownDuration, idStr) {
  const [players, setPlayers] = useState({});
  const roomRef = useRef(null);
  const [myId, setMyId] = useState(null);
  const [countdown, setCountdown] = useState(countdownDuration);
  const [identity, setidentity] = useState(idStr);

  useEffect(() => {
    const client = new Client("https://colys-blindate.onrender.com");
    client
      .joinOrCreate("game_room", { timer: countdownDuration, room: idStr })
      .then((room) => {
        room.onStateChange((state) => {
          setidentity(state.ID);
